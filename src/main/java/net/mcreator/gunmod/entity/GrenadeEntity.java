...
package net.mcreator.gunmod.entity;
...
import net.mcreator.gunmod.procedures.GrenadeProjectileHitsBlockProcedure;
import net.mcreator.gunmod.init.GunModModItems;
import net.mcreator.gunmod.init.GunModModEntities;
...
public class GrenadeEntity extends AbstractArrow implements ItemSupplier {
	public GrenadeEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(GunModModEntities.GRENADE.get(), world);
	}
	...
	public ItemStack getItem() {
		return new ItemStack(GunModModItems.GRENADEITEM.get());
	}
	...
	public void onHitBlock(BlockHitResult blockHitResult) {
		...
		GrenadeProjectileHitsBlockProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ());
	}
	...
	public static GrenadeEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		GrenadeEntity entityarrow = new GrenadeEntity(GunModModEntities.GRENADE.get(), entity, world);
	...
	public static GrenadeEntity shoot(LivingEntity entity, LivingEntity target) {
		GrenadeEntity entityarrow = new GrenadeEntity(GunModModEntities.GRENADE.get(), entity, entity.level());
		double dx = target.getX() - entity.getX();
		double dy = target.getY() + target.getEyeHeight() - 1.1;
		double dz = target.getZ() - entity.getZ();
		entityarrow.shoot(dx, dy - entityarrow.getY() + Math.hypot(dx, dz) * 0.2F, dz, 0.88f * 2, 12.0F);
		entityarrow.setSilent(true);
